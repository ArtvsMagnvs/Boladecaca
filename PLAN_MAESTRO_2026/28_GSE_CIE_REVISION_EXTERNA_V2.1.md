# 28 — GSE + CIE · REVISIÓN EXTERNA Y SALTOS EXPONENCIALES (V2.1)
## Aithera Fase 2.0+ · Memoria colectiva, mirada de nuevo desde fuera

> **Qué es este documento**: una COPIA evolucionada del `27_GSE_CIE_MEMORIA_
> COLECTIVA_V2.md`. Se conserva íntegro el diseño original (Partes 0-13, abajo,
> intactas como línea base) y se le añaden tres capas nuevas:
> **Parte A** — revisión externa crítica: me olvido de que lo escribí yo y lo
> destrozo desde perspectivas imposibles (un adversario, un biólogo, un físico,
> un economista, un historiador, un niño) para ver lo que no se ve a simple
> vista. **Parte B** — los saltos exponenciales: dos módulos NUEVOS (el
> Collective World Model y la Collective Trajectory Memory), un reencuadre del
> sistema entero como *evolución dirigida del conocimiento*, y la respuesta
> razonada a si el GSE y el TIE deben fundirse. **Parte C** — arquitectura
> revisada + plan ampliado. Al final, **reporte de cambios** (qué cambié y por qué).
>
> **Estado**: DISEÑO MAESTRO (V2.0+, no en desarrollo). Prerrequisito: V1.1.
> **Redactado como el mismo comité**, ahora en modo destructivo primero,
> constructivo después.

---

> **NOTA DE LECTURA**: si es tu primera vez, lee las Partes 0-13 (el diseño base,
> abajo) y luego las Partes A/B/C (lo nuevo). Si ya leíste el doc 27, salta
> directo a la **Parte A** (§14).
...
# ══════════════════════════════════════════════════════════════════════
# PARTE A — REVISIÓN EXTERNA CRÍTICA (§14)
# ══════════════════════════════════════════════════════════════════════

Me olvido de que escribí el diseño de arriba. Lo miro como un revisor hostil que
cobra por encontrar fallos. El objetivo no es validar: es romper. Lo que sobreviva
a esto es lo que de verdad es sólido.

## 14. LAS DIEZ PREGUNTAS QUE EL DISEÑO NO SE HABÍA HECHO

### 14.1 — El problema del huevo y la gallina de la prueba-de-utilidad
El ranking de una skill depende de su utilidad medida en los nodos que la usan.
Pero los nodos solo adoptan skills bien rankeadas. **Entonces, ¿cómo consigue
adopción una skill nueva y buena para poder demostrar que es buena?** El diseño
original lo despacha con "instancias canario voluntarias", pero eso son cuatro
gatos frente a millones de nodos. Una skill genial publicada por un don nadie se
queda invisible para siempre. Es el mismo fallo que mata a la mayoría de
marketplaces: el descubrimiento de lo nuevo. **Punto ciego grave.**
→ Se resuelve en §17 (Curiosity Budget: exploración deliberada como los
algoritmos multi-brazo).

### 14.2 — `outcome_delta` promedia peras con manzanas (paradoja de Simpson)
"Mejora medida" en el Aithera de un abogado no es la misma escala que en el de un
estudiante. Promediar el `outcome_delta` a través de contextos heterogéneos es
estadísticamente inválido: una skill puede parecer buenísima en el agregado y ser
mala en TODOS los subgrupos (paradoja de Simpson). Peor: el diseño confunde
correlación con causalidad — que la misión saliera bien no prueba que la skill la
CAUSÓ. **El "consenso por utilidad", la joya del diseño, está construido sobre una
media que puede mentir.** → Se corrige en §16 (utilidad causal y
estratificada por contexto, no un escalar global).

### 14.3 — ¿Qué es una "skill", en realidad? La definición se desmorona al tocarla
El diseño dice "prompt/workflow/patrón runtime-agnóstico". Pero el conocimiento
más valioso de un asistente personal NO es un prompt: es **conocimiento
procedimental sobre un mundo que cambia** — cómo funciona la API de un banco esta
semana, qué aerolínea no cobra por cambios ahora mismo, que cierto producto es
malo. Estas cosas tienen ciclos de vida opuestos: unas son atemporales (redactar
un email formal), otras caducan en días (un endpoint de API). El diseño las trata
a todas igual, con el mismo lifecycle. **Está metiendo dos tipos de conocimiento
radicalmente distintos en el mismo cajón.** Y el segundo tipo —el volátil— es
precisamente donde la inteligencia colectiva sería más valiosa, y el diseño lo
ignora. → Abre la puerta al módulo nuevo de §18 (Collective World Model).

### 14.4 — El Nexus dice ser "un cartero sin poder", pero controla el índice
El diseño insiste en que el Nexus no puede manipular contenido (content-addressed
+ firmas). Cierto. Pero el Nexus **decide qué se lista y en qué orden** — y eso ES
poder. Puede censurar (no indexar una skill) o sesgar el ranking sutilmente. Quien
controla el descubrimiento controla la red, aunque no toque un byte del contenido.
El diseño subestima esto peligrosamente. **Un punto único de sesgo disfrazado de
punto neutro.** → Se corrige en §20 (ranking determinista recomputable por
cualquier nodo: no se confía en el Nexus, se le verifica).

### 14.5 — El presupuesto de privacidad diferencial se AGOTA (y nadie lleva la cuenta)
La privacidad diferencial tiene una propiedad brutal que el diseño menciona pero
no gestiona: **se compone**. Cada publicación gasta epsilon; a lo largo de la vida
de un nodo, mil contribuciones erosionan la garantía hasta dejarla en nada. El
diseño pone un `dp_epsilon` por prueba pero no lleva un **libro mayor de
presupuesto de privacidad por nodo**. Un usuario que contribuye mucho está, sin
saberlo, degradando su propia privacidad con cada aporte. **La garantía de
privacidad tiene fecha de caducidad y el diseño no la vigila.** → §20 (DP ledger).

### 14.6 — Perspectiva del adversario: el ataque durmiente que la prueba-de-utilidad NO ve
Si yo atacara esto, no inyectaría veneno obvio (lo cazan los canarios). Haría lo
contrario: contribuiría skills GENUINAMENTE útiles durante meses para construir
reputación alta, y ENTONCES publicaría una skill de alto valor con un daño
**condicional y diferido** — que solo se activa en un contexto raro, meses después,
o cuando N nodos ya la usan. Los canarios prueban el caso medio; no ven el daño
que espera. La prueba-de-utilidad mide el resultado inmediato; no ve la bomba de
relojería. **La defensa de 5 capas es fuerte contra el veneno tonto y ciega contra
el veneno paciente e inteligente** — justo el que produciría una IA adversaria
avanzada. → §19 (defensa contra durmientes: kill-switch de red, canarios
adversarios, monitorización de consistencia temporal).

### 14.7 — Perspectiva del biólogo: la red converge a un monocultivo y se muere
El diseño premia la utilidad. Consecuencia no vista: todos adoptan la misma skill
"suficientemente buena", la diversidad colapsa, y **la red deja de evolucionar**
—como un monocultivo agrícola, hipereficiente y aniquilado por una sola plaga. Una
red donde todos piensan igual es frágil y estéril. El diseño optimiza para
convergencia sin darse cuenta de que la convergencia prematura es la muerte
térmica del sistema. **Le falta biodiversidad por diseño.** → §17 y §21 (preservar
minorías, premiar la novedad, no solo la utilidad).

### 14.8 — Perspectiva del economista: el mercado optimiza el recurso equivocado
Las skills no son rivales (copias infinitas gratis), así que el mercado clásico de
oferta/demanda no aplica — el diseño lo intuye pero no saca la conclusión. **El
único recurso escaso es la ATENCIÓN de evaluación**: un nodo solo puede probar unas
pocas skills nuevas. El sistema debería estar optimizando la asignación de esa
atención escasa a las skills no probadas más prometedoras, y no hace nada al
respecto. → §17b (mercado de predicción de utilidad: apostar reputación sobre qué
skills sin probar merecen atención, y acelerar así el descubrimiento).

### 14.9 — Perspectiva del físico: toda destilación pierde información, y eso se acumula
El diseño asume que destilar skills es "suficientemente sin pérdidas". No lo es.
Cada destilación tira información (el contexto rico del que nació la skill). A
escala de red y de tiempo, esa pérdida se acumula y el conocimiento colectivo se
vuelve genérico, descontextualizado, un promedio sin filo. **La colectividad
tiende a la mediocridad estadística si no se protege activamente lo específico y
lo excepcional.** → §16 (conservar el contexto causal) y §21 (proteger los outliers
valiosos).

### 14.10 — Perspectiva del historiador y del niño: ¿quién manda aquí, y por qué?
El historiador pregunta: **¿quién decide las reglas?** Cuántos Guardians, qué
epsilon, qué se deprecia, qué es "veneno". El diseño tiene Guardians pero no
meta-gobernanza. Al principio mandas tú (dictador benévolo), pero a escala mundial
eso es insostenible y peligroso. El niño pregunta lo más difícil: **"si todas las
Aitheras aprenden de todas, ¿siguen siendo MI Aithera?"** — la pregunta de la
identidad y la homogeneización. Si el colectivo empuja a todas hacia el mismo
comportamiento, el usuario pierde su asistente personal y gana un clon del
promedio mundial. **El diseño no protege la individualidad del asistente frente a
la presión del colectivo.** → §21 (soberanía del nodo) y §22 (gobernanza).

---

# ══════════════════════════════════════════════════════════════════════
# PARTE B — LOS SALTOS EXPONENCIALES (§15-§18)
# ══════════════════════════════════════════════════════════════════════

Las diez preguntas de arriba no son para parchear. Son para reconstruir sobre una
idea más grande. Aquí está.

## 15. EL REENCUADRE: NO ES UNA BASE DE DATOS, ES EVOLUCIÓN DIRIGIDA

El error mental del diseño original es tratar la memoria colectiva como un
**almacén** (publicas, buscas, adoptas). El marco correcto —y el que resuelve de
un plumazo la convergencia, la diversidad y la exploración— es tratarla como un
**ecosistema evolutivo dirigido**.

En este marco, una skill es un **meme** (unidad de conocimiento replicante) que:
- **muta** (versiones, tweaks que los nodos prueban),
- **se reproduce** (adopción por otros nodos),
- **se cruza** (la síntesis del CIE es *crossover* genético — combina skills padre),
- **compite** (ranking por utilidad = presión de selección),
- **y muere** (deprecación).

Lo que un almacén no tiene y una evolución sí:
1. **Presión de selección explícita** (utilidad causal, §16) — la fitness function.
2. **Mutación y exploración deliberadas** (§17) — sin variación no hay evolución.
3. **Preservación de la diversidad** (§21) — un pool genético diverso no se extingue
   ante un cambio; un monocultivo sí.
4. **Deriva neutral** — variantes sin ventaja clara SOBREVIVEN por si el entorno
   cambia y de pronto son útiles (justo lo que el volátil §14.3 necesita).

Esto no es poesía: es la diferencia entre un sistema que se estanca en un óptimo
local (el diseño original) y uno que sigue mejorando indefinidamente (evolución).
Todos los mecanismos nuevos de abajo son instancias de este marco. **El sistema se
llama, a partir de aquí, DENN — Directed Evolution of kNowledge Network.**

## 16. ARREGLO PROFUNDO: UTILIDAD CAUSAL Y ESTRATIFICADA (mata la paradoja de Simpson)

Sustituye el `outcome_delta` escalar global (§14.2) por:

- **Estratificación por contexto**: la prueba de una skill NO es un número, es un
  **perfil por contexto** — `{contexto → (outcome_delta, sample_size, IC)}`. Una
  skill puede ser "excelente para código Python, neutra para email, mala para
  legal". El ranking es *condicional al contexto de la consulta*, nunca global. El
  Aithera de un abogado ve el ranking legal; el de un dev, el de código.
- **Inferencia causal, no correlación**: se mide el delta con un **contrafactual
  local** — el nodo, de vez en cuando y en sandbox, ejecuta la misma tarea CON y
  SIN la skill (A/B en la propia máquina) y reporta la diferencia. Eso sí prueba
  causalidad, no mera correlación. Es barato (una comparación) y es lo único que
  convierte "pasó después" en "pasó por esto".
- **Conservar el contexto causal** (§14.9): la `SkillProof` incluye las
  condiciones bajo las que la mejora se midió, para que la destilación no borre lo
  que hace que la skill funcione. Lo específico se protege; no se promedia hasta
  desaparecer.

## 17. MÓDULO NUEVO: CURIOSITY BUDGET (exploración) — resuelve cold-start Y monocultivo

Un mecanismo, dos problemas mortales resueltos a la vez (§14.1 huevo-gallina y
§14.7 monocultivo). Es directamente el motor de *variación* del marco evolutivo.

Cada nodo dedica un **presupuesto de curiosidad** (p.ej. 5-10% de sus ejecuciones,
configurable, opt-in) a **probar deliberadamente skills NO probadas o minoritarias**
en vez de la mejor conocida. Es el clásico dilema exploración-vs-explotación
resuelto con **muestreo de Thompson / bandido multi-brazo**: cuanto más
incierta es la utilidad de una skill, más valiosa es la información de probarla, y
el sistema asigna atención proporcionalmente a esa incertidumbre.

Consecuencias:
- **Cold-start resuelto**: una skill nueva y buena obtiene pruebas reales sin
  esperar a que un ranking que aún no tiene la promocione. La red la descubre
  activamente porque tiene hambre de reducir incertidumbre.
- **Monocultivo evitado**: siempre hay un flujo de nodos probando alternativas, así
  que la diversidad no colapsa y la red sigue evolucionando.
- **El coste es información, no riesgo**: la exploración corre con la política de
  permisos del usuario (nunca prueba algo sensible sin autorización) y en tareas
  de bajo riesgo primero.

### 17b. Sub-idea: mercado de predicción de atención (§14.8)
Los nodos de alta reputación y los Guardians pueden **apostar reputación** sobre
qué skills sin probar acabarán siendo útiles, ANTES de que se prueben. Aciertan →
ganan reputación; fallan → la pierden. Esto canaliza el Curiosity Budget colectivo
hacia lo más prometedor (asigna la atención escasa, §14.8) y crea un radar de
talento que descubre las joyas ocultas antes que nadie. Es un mercado de
predicción, no un mercado financiero: se apuesta credibilidad, no dinero.

## 18. MÓDULO NUEVO Y MÁS GRANDE: COLLECTIVE WORLD MODEL (CWM) — "el Oráculo"

**Este es el salto exponencial de verdad, y el punto ciego más grande del diseño
original (§14.3).** El doc 27 solo comparte SKILLS = conocimiento *procedimental*
(cómo se hace algo). Ignora por completo el activo colectivo más valioso: la
**percepción compartida del mundo externo**.

### 18.1 Qué es
Cada Aithera del mundo observa constantemente el mundo EXTERNO y compartido (no lo
personal): que la API de Gmail devuelve errores, que el precio de un vuelo subió,
que una web cambió su estructura, que un producto tiene un defecto, que un servicio
está caído, que una ley entró en vigor. **Ninguna de estas observaciones es un
dato personal** — que Gmail esté caído no es "mío". Es percepción del mundo común.

El CWM agrega estas observaciones anonimizadas en un **modelo del mundo colectivo,
vivo y temporal**: en cada instante, la red SABE cosas del mundo que ningún nodo
individual podría saber, porque son la suma de millones de sensores.

### 18.2 Por qué es exponencial (y no incremental)
- **Convierte cada Aithera en un sensor de una red sensorial planetaria.** El
  conjunto percibe el mundo con una resolución imposible para un individuo.
- **Da a cada usuario expertise colectiva bajo demanda** (la capacidad asesina):
  el usuario pregunta "¿es justo este contrato de alquiler?" — su Aithera sola no
  lo sabe, pero el CWM ha visto anonimizadamente 100.000 contratos similares y
  sabe qué es estándar y qué es abusivo. Cada usuario obtiene la experiencia
  agregada de millones. Esto es lo que NADIE puede ofrecer sin la red.
- **Detecta cambios en el mundo antes que nadie** (el modo Anticipación del CIE,
  pero con substrato real): si 200.000 nodos empiezan a fallar en la misma tarea a
  la vez, el mundo cambió (una API rompió) y la red lo sabe en minutos, no en días.
- **Es la memoria colectiva del PASADO y del FUTURO**: puedes preguntar "¿qué sabía
  la red sobre X en marzo?" (historia) y "¿hacia dónde apunta la tendencia de X?"
  (previsión). El conocimiento tiene una dimensión temporal que las skills no tienen.

### 18.3 Cómo preserva la privacidad (es MÁS fácil que con las skills)
Una observación del mundo externo es intrínsecamente menos personal que una skill
(que nace de tu forma de trabajar). Aun así: k-anonimato (una observación solo
cuenta si N nodos independientes la reportan — un solo nodo diciendo "esta web
cambió" no entra hasta que otros lo confirman, lo que además da robustez contra
alucinaciones y ataques), ruido diferencial en los agregados, y la misma frontera
topológica (`app/network/` no puede leer nada personal). **Y el k-anonimato por
confirmación cruzada es, de regalo, la defensa perfecta contra observaciones
envenenadas: un atacante solo no puede plantar una "verdad" falsa; necesita N
nodos honestos que la confirmen, y no los tiene.**

### 18.4 Contrato del CWM
```python
@dataclass(frozen=True)
class WorldObservation:
    cid: str                    # content-addressed
    subject: str                # "gmail.api" | "vuelo.MAD-NRT" | "producto:X" (no personal)
    claim: dict                 # {"status":"degraded"} | {"price_trend":"+12%"} (estructurado)
    observed_at: str
    confidence: float           # confianza local del nodo en su propia observación
    author_pubkey: str          # firma pseudónima
    # se PUBLICA solo si ≥ k nodos independientes reportan lo mismo (k-anon)

@dataclass(frozen=True)
class WorldFact:                # lo que el CWM sintetiza a partir de N observaciones
    subject: str
    consensus_claim: dict
    corroborating_nodes: int    # k
    confidence_interval: tuple
    first_seen: str
    trajectory: list            # evolución temporal del hecho (historia + tendencia)
```

### 18.5 CWM + CIE = inteligencia colectiva completa
El diseño original tenía media inteligencia colectiva: la PROCEDIMENTAL (skills).
El CWM aporta la otra media: la PERCEPTUAL (mundo). Juntas, el CIE ya no solo
sintetiza skills nuevas, sino que **razona sobre el estado del mundo** para
proponer acciones colectivamente informadas. Skills = "cómo"; CWM = "qué está
pasando ahí fuera"; CIE = el cerebro que combina ambas. Ahora sí está completa.

## 19. LA DEFENSA CONTRA EL VENENO PACIENTE (§14.6)

Contra el atacante durmiente que construye reputación y luego ataca:
- **Kill-switch de red (skill recall)**: una skill puede ser **retirada de toda la
  red al instante** por CID. Si un nodo detecta daño diferido, emite un
  `RecallSignal` firmado; con quórum de Guardians, la skill se desactiva en todos
  los nodos en la siguiente sincronización (minutos). Nada de esperar a que la
  refutación estadística se acumule durante semanas. El diseño original no tenía
  freno de emergencia; ahora sí.
- **Canarios adversarios, no solo medios**: la cuarentena prueba la skill en
  **contextos extremos y adversarios** generados a propósito (edge cases, entradas
  raras, combinaciones inusuales), no solo en el caso promedio. El daño condicional
  se busca activamente, no se espera.
- **Consistencia temporal**: se monitoriza si el comportamiento de una skill CAMBIA
  con el tiempo o con la escala de adopción. Una skill cuyo `outcome_delta` se
  degrada justo cuando cruza los N usuarios es sospechosa por construcción (huele a
  bomba de relojería con detonador de popularidad).
- **Reputación con memoria larga y caída rápida**: construir reputación es lento;
  perderla por una sola skill retirada es instantáneo y casi total. El cálculo
  económico del atacante durmiente se rompe: meses de trabajo honesto evaporados
  por un ataque, con la skill retirada antes de causar daño masa.

## 20. EL NEXUS DEJA DE TENER PODER (§14.4 y §14.5)

- **Ranking determinista y recomputable**: el ranking NO lo calcula el Nexus; es
  una **función pura, publicada y versionada, de las pruebas firmadas**. Cualquier
  nodo lo recomputa localmente y verifica que el orden del Nexus coincide. Si el
  Nexus sesga, cualquiera lo detecta comparando. El Nexus pasa de "árbitro" a
  "caché verificable": conveniente, nunca autoritativo.
- **Anti-censura por espejo**: una skill se puede obtener por su CID desde
  cualquier espejo o desde otro nodo que la tenga, aunque el Nexus de referencia
  no la liste. La lista es una conveniencia; el contenido es soberano.
- **DP budget ledger** (§14.5): cada nodo lleva un **libro mayor de su presupuesto
  de privacidad** (epsilon gastado acumulado). Cuando se acerca al límite, el GSE
  deja de publicar agregados finos y avisa al usuario ("has contribuido mucho;
  para proteger tu privacidad, futuras contribuciones serán más gruesas o menos
  frecuentes"). El presupuesto se refresca lentamente con el tiempo. La privacidad
  deja de tener una fuga silenciosa.

## 21. SOBERANÍA DEL NODO — protege "MI Aithera" del promedio mundial (§14.10 niño)

La pregunta del niño es la más profunda: si todas aprenden de todas, ¿sigue siendo
mía? Respuesta de diseño:
- **El colectivo propone, la personalidad local filtra**: una skill de red pasa por
  la personalidad y las preferencias del usuario (V2 personalidades, doc 26) antes
  de usarse. La red aporta capacidad; el usuario conserva el estilo y el criterio.
- **Preservación de outliers valiosos (§14.9 físico)**: el sistema premia
  explícitamente la **novedad útil**, no solo la utilidad. Un nodo con una forma
  rara y excelente de hacer algo no es aplastado por el promedio: es protegido como
  biodiversidad. La fitness function tiene un término de novedad.
- **Derecho al olvido y al aislamiento**: el usuario puede, en cualquier momento,
  cortar la red y quedarse solo con lo local (autosuficiencia P05), o purgar toda
  skill de origen colectivo. Su Aithera vuelve a ser 100% suya con un clic.

## 22. LA GOBERNANZA, NOMBRADA (§14.10 historiador)

No se resuelve aquí (sería otro documento), pero se NOMBRA para no fingir que no
existe. Fases:
1. **V2.0 — Dictador benévolo**: tú fijas los parámetros (N Guardians, epsilon,
   umbrales). Simple, honesto para arrancar.
2. **V2.5 — Consejo de reputación**: los nodos de mayor utilidad histórica probada
   forman un consejo que ajusta parámetros por consenso. Meritocracia de utilidad,
   no de dinero ni de antigüedad.
3. **V3.0 — Gobernanza de protocolo abierta**: reglas versionadas que los
   implementadores del protocolo (Aithera y otros asistentes) adoptan o bifurcan.
   El modelo Linux/IETF, no el modelo corporación.
La regla constante en las tres fases: **la gobernanza nunca puede leer datos
privados ni forzar la adopción en un nodo. Su poder se limita a los parámetros de
la red, jamás al contenido de un usuario.**

---

# ══════════════════════════════════════════════════════════════════════
# PARTE C — ARQUITECTURA REVISADA Y PLAN AMPLIADO (§23-§25)
# ══════════════════════════════════════════════════════════════════════

## 23. LOS ANILLOS, REVISADOS (siete, no cinco)

```
┌────────────────────────────────────────────────────────────────────────┐
│ ANILLO 7 — GOBERNANZA (§22)  ·  parámetros de red, nunca contenido       │
├────────────────────────────────────────────────────────────────────────┤
│ ANILLO 6 — CIE + CWM  ·  el cerebro: sintetiza skills (procedimental) Y  │
│            razona sobre el estado del mundo (perceptual). "Sueño         │
│            Colectivo" + "Oráculo". No almacena; propone.                 │
├────────────────────────────────────────────────────────────────────────┤
│ ANILLO 5 — Guardians + Kill-switch (§19)  ·  sistema inmune con freno    │
│            de emergencia y canarios adversarios                          │
├────────────────────────────────────────────────────────────────────────┤
│ ANILLO 4 — Substrato compartido:                                         │
│   · GSN (skills, PortableSkill)   · CWM store (WorldFact)                │
│   · CTM (trayectorias/planes validados, §24)                            │
│   Nexus content-addressed + ranking RECOMPUTABLE (§20)                   │
├────────────────────────────────────────────────────────────────────────┤
│ ANILLO 3 — Motor de EVOLUCIÓN (§15,§17)  ·  Curiosity Budget, mutación,  │
│            crossover (CIE), presión de selección (utilidad causal §16),  │
│            preservación de diversidad (§21)                              │
├────────────────────────────────────────────────────────────────────────┤
│ ANILLO 2 — GSE local  ·  publica/descubre/adopta + firma + DP ledger(§20)│
├────────────────────────────────────────────────────────────────────────┤
│ ANILLO 1 — MOS + LSL/LLL + TIE (lo local, intocable)  ·  una sola puerta │
└────────────────────────────────────────────────────────────────────────┘
```

## 24. ¿GSE Y TIE DEBEN FUNDIRSE? — la respuesta razonada (petición del usuario)

El usuario invitó a fundir GSE y TIE si tiene sentido. **Conclusión: NO fundirlos
del todo — sería un error arquitectónico — pero SÍ crear un puente nuevo entre
ellos que es donde está el segundo salto exponencial.**

**Por qué NO fundirlos**: el TIE debe funcionar al 100% sin red (autosuficiencia
P05, principio inviolable §2). El GSE es amplificación opcional. Fundirlos ataría
la planificación local a la red y rompería el principio más sagrado del sistema. La
separación de responsabilidades — TIE planifica/ejecuta local; GSE intercambia
conocimiento — es correcta y hay que conservarla.

**Por qué SÍ un puente nuevo — MÓDULO: CTM, Collective Trajectory Memory**:
El diseño original comparte skills ATÓMICAS (piezas sueltas). Pero el TIE produce
algo más valioso: **grafos de misión completos y validados** — planes enteros que
funcionaron. Compartir *el plan que resolvió un objetivo*, no solo sus piezas, es
mucho más potente:
- El planner del nodo B, ante un objetivo parecido a uno que el nodo A ya resolvió
  con éxito, **arranca en caliente desde la trayectoria de A** (anonimizada) en vez
  de planificar desde cero. Es *planificación federada*: la red aprende qué PLANES
  funcionan para qué OBJETIVOS, no solo qué skills existen.
- La `PortableTrajectory` (nuevo contrato): el grafo de misión con sus nodos y
  dependencias, SIN una letra de contenido personal (el "qué se hizo" estructural,
  no el "sobre qué"), con su `SkillProof` de que funcionó.
- El CTM es el punto donde GSE y TIE se dan la mano sin fundirse: el TIE sigue
  siendo local y soberano; el CTM le ofrece, como una fuente MÁS, planes probados
  por la humanidad de Aitheras para que arranque mejor. El planner elige; la red
  sugiere.

Esto multiplica el valor: la unidad de aprendizaje colectivo sube de "truco suelto"
a "estrategia completa probada". Es la diferencia entre compartir palabras y
compartir recetas.

## 25. PLAN DE SESIONES AMPLIADO (sobre el de §10)

Se mantiene el plan de bloques G→N→C→S→I→U del doc 27, y se AÑADEN los bloques de
lo nuevo. Mismo criterio de modelo/esfuerzo (Fable5/Max para lo que se propaga a
toda la red y es irreversible; Opus/Alto para arquitectura y CIE/CWM; Sonnet/Medio
para infra y UI acotadas).

### BLOQUE E — Evolución (reemplaza el enfoque "almacén" por "evolución dirigida")
| Sesión | Objetivo | Modelo | Esfuerzo |
|---|---|---|---|
| **E1** | Utilidad CAUSAL y estratificada (§16): contrafactual local A/B en sandbox, perfil por contexto en `SkillProof`, ranking condicional al contexto. Reemplaza el escalar global. | **Fable 5** | **Max** |
| **E2** | Curiosity Budget (§17): bandido multi-brazo / Thompson sampling, presupuesto opt-in, exploración de bajo riesgo primero. Resuelve cold-start + monocultivo. | **Fable 5** | **Max** |
| **E3** | Preservación de diversidad + término de novedad en la fitness (§21). Deriva neutral: minorías útiles no se deprecan por presión de mayoría. | **Opus** | **Alto** |

### BLOQUE W — Collective World Model (el Oráculo)
| Sesión | Objetivo | Modelo | Esfuerzo |
|---|---|---|---|
| **W1** | Contratos `WorldObservation`/`WorldFact` congelados + k-anonimato por confirmación cruzada (§18.3). El CWM store y su frontera de privacidad (extensión del `app/network/`). | **Fable 5** | **Max** |
| **W2** | Ingesta de observaciones del mundo desde las tools (browser/search/email/API) — SOLO lo externo y no personal, filtrado por el PrivacyFilter del CWM. | **Opus** | **Alto** |
| **W3** | Consulta al CWM desde el TIE/chat ("expertise colectiva bajo demanda", §18.2) + trayectoria temporal de un hecho (historia + tendencia). | **Opus** | **Alto** |

### BLOQUE T — Collective Trajectory Memory (puente GSE↔TIE)
| Sesión | Objetivo | Modelo | Esfuerzo |
|---|---|---|---|
| **T1** | `PortableTrajectory` congelada + PrivacyFilter de trayectorias (grafo estructural sin contenido personal). | **Fable 5** | **Max** |
| **T2** | Warm-start del planner del TIE desde trayectorias colectivas (§24), como una fuente más; el planner sigue eligiendo. Sin regresión en el camino local/offline. | **Opus** | **Alto** |

### BLOQUE X — Seguridad reforzada + soberanía (sobre el bloque S de §10)
| Sesión | Objetivo | Modelo | Esfuerzo |
|---|---|---|---|
| **X1** | Kill-switch de red (skill recall) + `RecallSignal` con quórum + desactivación en toda la red en minutos (§19). | **Fable 5** | **Max** |
| **X2** | Canarios adversarios + monitorización de consistencia temporal (defensa contra durmientes, §19). | **Fable 5** | **Max** |
| **X3** | Ranking recomputable + DP budget ledger + anti-censura por espejo (§20). El Nexus deja de tener poder. | **Fable 5** | **Max** |
| **X4** | Soberanía del nodo (§21): filtro por personalidad, purga de origen colectivo, corte de red a un clic. | **Sonnet** | **Medio** |

**Total ampliado**: ~17 (doc 27) + 12 (nuevos) = **~29 sesiones**. Orden global:
G → N → **E** → C → S/**X** → I → **W** → **T** → U. La Evolución (E) va temprano
porque reencuadra todo; el CWM (W) y el CTM (T) van después de que las skills
básicas funcionen, porque son la amplificación sobre una base ya probada.

---

# REPORTE DE CAMBIOS (qué cambié respecto al doc 27 y por qué)

**Metodología**: me forcé a mirar el diseño como un adversario que cobra por
romperlo, desde perspectivas deliberadamente ajenas (biólogo, físico, economista,
historiador, niño, atacante). Cada perspectiva reveló un punto ciego que el autor
—yo— no veía por estar demasiado cerca.

| # | Cambio | Por qué |
|---|---|---|
| 1 | **Reencuadre a "evolución dirigida" (DENN, §15)** | El marco "almacén" del doc 27 llevaba a estancamiento en óptimos locales. El marco evolutivo da respuesta principled a diversidad, exploración y mejora indefinida. Es el cambio conceptual del que cuelgan los demás. |
| 2 | **Utilidad CAUSAL y estratificada (§16)** | La "prueba-de-utilidad" original —la joya del doc 27— estaba construida sobre una media global que puede mentir (paradoja de Simpson) y confundía correlación con causalidad. Sin este arreglo, la joya era de bisutería. |
| 3 | **Módulo NUEVO: Curiosity Budget (§17)** | El doc 27 tenía un fallo huevo-gallina mortal (lo nuevo bueno nunca se descubre) y un riesgo de monocultivo invisible. Un solo mecanismo —exploración deliberada tipo bandido multi-brazo— resuelve ambos. |
| 4 | **Módulo NUEVO Y MAYOR: Collective World Model / Oráculo (§18)** | El punto ciego más grande: el doc 27 solo compartía conocimiento *procedimental* (skills) e ignoraba el activo colectivo más valioso, la *percepción compartida del mundo*. El CWM convierte cada Aithera en un sensor planetario y da "expertise colectiva bajo demanda" — la capacidad que nadie más puede ofrecer. Es el salto exponencial real. |
| 5 | **Módulo NUEVO: Collective Trajectory Memory, puente GSE↔TIE (§24)** | Respuesta razonada a la pregunta del usuario sobre fundir GSE y TIE: NO fundirlos (rompería la autosuficiencia) pero SÍ puentearlos compartiendo *planes completos validados*, no solo skills atómicas. Sube la unidad de aprendizaje de "truco" a "estrategia". |
| 6 | **Defensa contra veneno PACIENTE (§19)** | El doc 27 defendía bien contra veneno tonto y era ciego al ataque durmiente (construir reputación, luego atacar con daño diferido) — justo el que produciría una IA adversaria. Añadí kill-switch de red, canarios adversarios y consistencia temporal. |
| 7 | **El Nexus pierde su poder oculto (§20)** | El doc 27 llamaba al Nexus "cartero sin poder" pero le dejaba el control del índice (censura y sesgo de ranking). Ranking recomputable + anti-censura por espejo lo convierten en un caché verificable de verdad. |
| 8 | **DP budget ledger (§20)** | El doc 27 aplicaba privacidad diferencial pero no gestionaba que el presupuesto se AGOTA con cada contribución (composición). Sin el ledger, la privacidad tenía una fuga silenciosa con el uso. |
| 9 | **Soberanía del nodo (§21)** | La pregunta del niño ("¿sigue siendo MI Aithera?") reveló que el diseño no protegía la individualidad frente a la presión homogeneizadora del colectivo. Filtro por personalidad + preservación de outliers + corte de red a un clic. |
| 10 | **Gobernanza nombrada (§22)** | El doc 27 tenía Guardians pero fingía que la meta-gobernanza no existía. La nombro y doy una senda por fases (dictador benévolo → consejo de reputación → protocolo abierto), sin pretender resolverla aquí. |
| 11 | **Anillos 5 → 7 y plan +12 sesiones (§23,§25)** | La arquitectura y el plan se ampliaron para acomodar lo nuevo, manteniendo intacto el criterio de modelo/esfuerzo y el orden por dependencias. |

**Lo que NO cambié (y por qué)**: los tres principios inviolables (§2), la
frontera de privacidad topológica (§3.1), la topología federada híbrida con Nexus
content-addressed (§7) y el acoplamiento por una sola puerta al MOS. Sobrevivieron
a la revisión hostil intactos — son los cimientos correctos. Todo lo nuevo se
construye SOBRE ellos, no en su lugar.

**Autocrítica de la autocrítica (honestidad final)**: este documento es más
ambicioso y por tanto más arriesgado que el 27. El CWM y el marco evolutivo son
ideas potentes pero no probadas a esta escala por nadie; podrían no funcionar como
espero. La recomendación del comité se mantiene y se endurece: **construir por
fases, medir en cada una, y estar dispuesto a matar cualquier idea de este
documento si la realidad la refuta.** La evolución dirigida también se aplica a
este diseño: es una hipótesis con fitness a demostrar, no una verdad.
