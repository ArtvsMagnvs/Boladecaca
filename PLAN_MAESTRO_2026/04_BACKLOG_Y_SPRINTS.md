...
| V1.0 | O1 intents+traces **+ contracts.py/graph.py del TIE** · O2 enricher+planner **(salida TaskGraph)** · O3 executor **(grafo lineal, checkpoint, kill-switch)** +gateway handler · O4 responder+LLL+UI · **E1-E2 MEL v1** (núcleo determinista + políticas + wizard + migración call-sites — doc 19) · O5 **cierre MVP beta** (auto-start, instalador, onboarding; tag v1.0.0-beta) | 14, 11, 19, 12 §3 |
| V1.1 | H0 investigación GO/NO-GO · H1-H4 runtime+adapters+LSL **+ Mission Learning + panel "lo aprendido"** | 10, 15 |
| V1.2 | MCP server/client · TIE v2 (olas, replan, missions, presupuestos) · **MEL v2** (Learning+Recommendation Engines + Custom builder drag&drop) · Skill Evolution · AutomationLearner · mission evals | 14 §5, 15 §6-9, 19 §12 |
