// components/settings/McpConnectForm.tsx — conectar UN servidor MCP (V1.2 C1b)
//
// El formulario que aparece al pulsar «Conectar» en el directorio. Recibe la
// config YA resuelta (del catálogo curado o del registro oficial) y solo pide
// lo que el servidor declara necesitar, con su etiqueta y su enlace de ayuda.
//
...
//      NOMBRES de las claves guardadas, C1).
...
export default function McpConnectForm({
  ...
  const connect = async () => {
    ...
    try {
      ...
      try {
        ...
        // El servidor queda guardado igualmente (puede ser un token mal
        // pegado): se dice qué pasó, no se deshace la conexión a su espalda.
  ...
  return (
    <div className="rounded-xl p-3 border border-accent/40 bg-accent/5 space-y-3">
      <div>
        <p className="text-sm font-medium text-ink">{target.title}</p>
        {target.description && (
          <p className="text-[11px] text-ink-dim mt-0.5">{target.description}</p>
        )}
      </div>
