...
export default function VoicePanel() {
    ...
    try {
      ...
      if (provider === "edgetts") {
        ...
        if (status.configured) {
          ...
          setVoiceLoadError("ElevenLabs no configurado. Define la API key más abajo.");
        }
      }
  ...
  const handlePreview = useCallback(async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "Error al sintetizar voz");
      setIsPlaying(false);
      setCoreState("idle");
    } finally {
      setIsLoading(false);
  ...
  const handleInstallKokoro = useCallback(async () => {
    try {
      ...
      setError(e instanceof Error ? e.message : "No se pudo lanzar la instalación de Kokoro.");
    }
  }, []);
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      {activeProvider === "kokoro" && kokoroStatus && !kokoroStatus.available && (
        <div className="rounded-xl border border-signal-warn/30 bg-signal-warn/10 p-3 flex items-start justify-between gap-3">
          <div className="min-w-0 flex-1">
            <p className="text-xs font-medium text-ink mb-0.5">Kokoro (voz local offline)</p>
      ...
      <div className="rounded-xl border border-base-700/60 bg-base-900/40 p-3">
        <h4 className="text-xs font-medium text-ink mb-2">Personalidad</h4>
        <PersonalityPicker />
      </div>
          ...
          <p className="text-sm text-ink-faint">
            {voices.length === 0
              ? (activeProvider === "elevenlabs"
                  ? "No hay voces. Configura la API key de ElevenLabs más abajo."
                  : activeProvider === "kokoro"
                    ? "Kokoro no está instalado — usa el botón 'Instalar Kokoro' de arriba."
              ...
              return (
                ...
                >
                  ...
                  {selectedVoice?.voice_id === voice.voice_id ? (
                    ...
                    <div className="mt-2 text-[11px] text-ink-faint">Clic para usar como principal</div>
                  )}
                </div>
              );
            })}
...
// Vista simplificada para cuando el provider es eSpeak.
function EspeakFallback({ onPlay, isPlaying }: { onPlay: () => void; isPlaying: boolean }) {
  return (
    <div className="rounded-xl border border-base-700/60 bg-base-900/40 p-6 flex flex-col items-center gap-4 text-center">
