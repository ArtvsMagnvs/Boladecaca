...
// mecánica de arrastre/resize/expandir. Recibe `t` (no puede usar el hook
// directamente: es una función pura, no un componente).
export function windowShortcuts(
  expanded: boolean,
  t: (key: string, vars?: Record<string, string | number>) => string,
): ShortcutEntry[] {
  if (expanded) {
    return [[t("workspace.help.dblClickHeader"), t("workspace.help.dblClickRestore")]];
  }
  return [
    [t("workspace.help.dragHeader"), t("workspace.help.moveCard")],
    [t("workspace.help.dblClickHeader"), t("workspace.help.expandFullscreen")],
    [t("workspace.help.dragEdges"), t("workspace.help.resize")],
  ];
}
...
export function HelpButton({ open, onToggle, extra = [], align = "right", title }: Props) {
  const t = useT();
  const resolvedTitle = title ?? t("workspace.help.title");
  return (
    <div className="relative shrink-0">
      <button
        ...
        title={t("workspace.help.keyboardTitle")}
        aria-label={t("workspace.help.ariaLabel")}
      >
        ?
      </button>
      {open && (
        <div className={`absolute top-7 ${align === "right" ? "right-0" : "left-0"} z-30 glass-surface rounded-xl border border-base-700 p-3 w-64 shadow-glass`}>
          <p className="text-[10px] uppercase tracking-wide text-ink-faint mb-2">{resolvedTitle}</p>
